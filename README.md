...
- **MCP/插件系统** -- 通过 JSON-RPC（stdio 或 HTTP）连接外部 MCP 服务器，支持无重启热重载。
- **运行时工具创建** -- 智能体可以在运行时编写、保存并加载新的 Python 工具 (`create_tool`)。
...
```
     ...
     | 26 个内置   |  | 三级流水线:  |  | cron + 单次 |
     | 工具 + 插件 |  | 压缩/去重/   |  | jobs.json   |
     | 系统 + MCP  |  | 检索         |  | 持久化/时区  |
     +------+------+  +------------+  +-------------+
            |
     +------v------+
     |mcp_client.py|  基于 stdio/HTTP 的 JSON-RPC
     | MCP 协议实现 |  命名空间: server__tool
     | 自动重连      |  支持热重载
     +-------------+
```
...
| 模块 | 角色 | 核心职责 |
| :--- | :--- | :--- |
| **`main.py`** | **入口/感官** | 运行 HTTP 服务器，监听 Webhook。它负责“听”（接收消息）、“看”（处理图片）、“存”（保存文件）。 |
| **`llm.py`** | **大脑/决策** | 核心逻辑。它把用户的意图转换成具体的行动计划，并管理对话的上下文。 |
| **`tools.py`** | **手脚/执行** | 包含 20 多个工具（搜索、读写文件、运行命令等）。大脑决定用哪个，它就去执行哪个。 |
| **`messaging.py`** | **嘴巴/反馈** | 专门负责把大脑生成的回复发回给用户（Telegram 或 Web 端）。 |
...
3.  **大脑思考 (LLM Loop)**:
    合并后的消息传给 `llm.py`。大脑会进行决策：
    - 用户问了什么？
    - 我有多少个工具（Tools）可以用？
    - 我需不需要调用工具来获取更多信息（如 `web_search`）？
4.  **循环执行 (Tool Use Loop)**:
    如果大脑决定使用工具，它会告知 `tools.py` 执行。执行结果返回给大脑，大脑再次思考：“现在的搜索结果够回答用户了吗？”。如果不够，它会继续调用工具，单次对话最多循环 **20 次**。
5.  **回复 (Send Message)**:
    当大脑生成最终回复后，通过 `messaging.py` 的接口将结果推送到你的手机或 Web 界面上。
...
2. **单文件工具** -- 增加能力 = 在 `tools.py` 中添加一个带有 `@tool` 装饰器的函数。
